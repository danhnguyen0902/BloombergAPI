	...
	private static void processResponseBar(Event evt, String security) throws Exception
	{
		...
		while(miter.hasNext())
		{
            ...
            if(isSecurityError)
            {
                ...
                errorContent.append("response error" + "\n");
            ...
            else
            {
				...
				for(int valueIndex = 0; valueIndex < elmBarTickDataArray.numValues(); valueIndex++)
				{
	                ...
	                responseContent.append(RunIntradayBarDataRequest.timeFmt.format(dtTick.calendar().getTime()) + "\n");
	                responseContent.append(String.format("\t open = $%,.2f", open) + "\n");
	                responseContent.append(String.format("\t high = $%,.2f", high) + "\n");
	                responseContent.append(String.format("\t low = $%,.2f", low) + "\n");
					responseContent.append(String.format("\t close = $%,.2f", close) + "\n");
	                ...
	                System.out.println();*/
					responseContent.append(String.format("\t numEvents = %,d", numEvents) + "\n");
					responseContent.append(String.format("\t volume = %,d", volume) + "\n");
					responseContent.append(String.format("\t value = %,.0f", value) + "\n");
